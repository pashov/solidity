...
void CommandLineInterface::assemble()
{
	solAssert(m_options.input.mode == InputMode::EVMAssemblerJSON);
...
void CommandLineInterface::handleCombinedJSON()
{
	...
	if (m_options.input.mode == InputMode::EVMAssemblerJSON)
	{
		string fileName = m_fileReader.sourceUnits().begin()->first;
		Json::Value& contractData = output[g_strContracts][fileName] = Json::objectValue;
		if (m_options.compiler.combinedJsonRequests->asm_)
			contractData[g_strAsm] = m_compiler->assemblyJSON(fileName);
		if (m_options.compiler.combinedJsonRequests->binary)
			contractData[g_strBinary] = m_compiler->object(fileName).toHex();
		...
		if (m_options.compiler.combinedJsonRequests->srcMapRuntime && m_compiler->runtimeSourceMapping(fileName))
			contractData[g_strSrcMapRuntime] = *m_compiler->runtimeSourceMapping(fileName);
	}
	else
	{
